...
def adjust_optim(optimizer, n_iter, warmup_steps):
    optimizer.param_groups[0]['lr'] = (word_embedding_size ** (-0.5)) * min(n_iter ** (-0.5),
                                                                            n_iter * (warmup_steps ** (-1.5)))
...
parser.add_argument('--batch-size-val', type=int, help='Batch Size for Validation', default=64)
...
parser.add_argument('--num-tf-layers', help="Number of the Transformer Layers", type=int, default=3)
parser.add_argument('--num-heads', help="Number of Multihead Attention Process Heads", type=int, default=2)
...
if encoder_type == 'resnet18' or encoder_type == 'vgg16':
    ...
    CNN_channels = 1280  # efficientnet encoder has 1280 featuremap depth
...
# fixed_image = "1010031975.jpg" # For flickr30k's Dataset Later
...
if mode == "train":
    ...
    if use_checkpoint:  # Load from oreviously saved model
    ...
    while epoch <= max_epochs:
        ...
        if poor_iters > max_poor_iters:
            ...
            utils.save_model_and_result(model_save_path, args.experiment_name, best_model, decoder_type, best_optimizer,
                                        best_epoch, best_bleu4, best_loss, best_metrics, test_metrics)
            break
        print("Predicted caption: ", predict(model, device, fixed_image))
        # Save Model every 10 epochs for backup
        # if epoch % 10 == 0:
        #     model.cpu()
